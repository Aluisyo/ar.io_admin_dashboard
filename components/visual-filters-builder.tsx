...
interface VisualFiltersBuilderProps {
  service: string
}

export function VisualFiltersBuilder({ service }: VisualFiltersBuilderProps) {
  const [viewMode, setViewMode] = useState<'visual' | 'code' | 'help'>('visual')
      ...
      if (response.ok) {
        ...
        Object.entries(data).forEach(([key, value]) => {
          const filterKey = key === 'unbundleFilter' ? 'ANS104_UNBUNDLE_FILTER' :
                           key === 'indexFilter' ? 'ANS104_INDEX_FILTER' :
                           key === 'webhookIndexFilter' ? 'WEBHOOK_INDEX_FILTER' :
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle>AR.IO Gateway Filters</CardTitle>
              <CardDescription>
                Edit existing filter configurations from your AR.IO node's .env file
              </CardDescription>
            </div>
